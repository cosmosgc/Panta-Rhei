ghost-role-information-emotional-support-name = "Apoio emocional, animal de estimação"

ghost-role-information-emotional-support-description =  Você é um animal de estimação de apoio emocional, leal ao seu dono, certifique-se de animá-los!

ghost-role-information-emotional-support-rules =  Você é um [color=lightpink][bold]"Apoio emocional, animal de estimação"[/bold][/color] Você não se lembra de nada que aprendeu como fantasma, você pode lembrar do conhecimento sobre o jogo em geral, como cozinhar, como usar objetos, etc. [color=red] NÃO[/color]  permite lembrar, digamos, o nome, aparência, etc. de seu personagem anterior.

ghost-role-information-clippy-name = Clippy

ghost-role-information-clippy-description = O leal trabalhador do representante da estação, cheira a papelão e papel.

ghost-role-information-jerma-name = Jerma.

ghost-role-information-jerma-description = Momento Pog
